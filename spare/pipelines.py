...
class SparePipeline:
    ...
    # Database setup
    conn = sqlite3.connect('spare.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `spare`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM spare WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO spare (date, title, link, content)"
